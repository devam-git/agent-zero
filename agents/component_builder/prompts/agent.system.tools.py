...
class ComponentBuilderTools(VariablesPlugin):
    def get_variables(self, file: str, backup_dirs: list[str] | None = None, **kwargs) -> dict[str, Any]:

        # print(f"🔧 Component Builder tool plugin called!")
        ...
        # Define allowed default tools for Component_builder
        allowed_tools = [
            'code_execution_tool', 
            'input',
            'response'
        ...
        # print(f"📝 Filtering tools for Component_builder profile")
        filtered_files = []
        for pf in prompt_files:
            tool_name = os.path.basename(pf)[18:-3]  # Extract tool name from agent.system.tool.NAME.md
            if "component_builder" in pf:
                # print(f"   ✅ Including Component_builder tool: {tool_name}")
